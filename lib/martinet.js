...
function Martinet(options) {
  ...
  debug('Config Options %', this.options);
  ...
  this.clientSocket.on('message', function(data) {
    ...
    self.execute(data, data, function() {
      self.clientSocket.send(Array.prototype.slice.call(arguments));
    });
  });
  ...
  this.statusSocket.on('message', function(data) {
    ...
    if(msg.set === 'progress') {
      self.setProgress(msg.taskId, msg.progress);
    }
  });
  ...
  this.workerSocket.on('message', function(frame, data) {
    ...
    if(msg.set === 'complete') {
      debug('Receiving response from [' + msg.taskId + '] with result [%j]', msg.result);
      self.setComplete(msg);
    } else if(msg.set === 'error') {
      debug('Receiving error response from [' + msg.taskId + '] with error [%j]', msg.error);
      self.setError(msg.taskId, msg.error);
    }
  });
...
Martinet.prototype.setProgress = function(taskId, progress) {
  ...
  .then(function(task) {
    return task.updateAttributes({
      ...
      if(self._onProgress) {
        self._onProgress(self.parseTask(task));
      }
    }, function(err) {
...
Martinet.prototype.setError = function(taskId, error) {
  debug('Setting error for task [' + taskId + '] with error: %j', error);
  ...
  .then(function(task) {
    return task.updateAttributes({
      ...
      return TaskLog.create({
        ...
        if(self._onError) {
          self._onError(self.parseTask(task));
        }
      }, function(err) {
...
Martinet.prototype.setComplete = function(msg) {
  debug('Completing task [' + msg.taskId + ']');
  var Task = this.models.Task;
  var self = this;
  return Task.findById(msg.taskId)
  .then(function(task) {
    return task.updateAttributes({
      complete: true,
      progress: 1.0,
      ...
      if(self._onComplete) {
        self._onComplete(self.parseTask(task));
      }
    }, function(err) {
      debug('Error completing task [' + msg.taskId + ']: ' + err);
    });
  }, function(err) {
    debug('Error unable to find taskId [' + msg.taskId + ']: ' + err);
  });
};
...
Martinet.prototype.parseTask = function(task) {
  var t = task.get();
  try {
    t.result = JSON.parse(t.result);
  } catch (e) { }
  try {
    t.description = JSON.parse(t.description);
  } catch (e) { }
  return t;
};
...
Martinet.prototype.taskStatus = function(opts) {
  ...
  .then(function(tasks) {
    return _.map(tasks, function(task) {
      return self.parseTask(task);
    });
  }, function(err) {
    debug('Error getting tasks for query %j: %j', opts, err);
  });
};
