...
function Worker(options) {
  ...
  var workerConnection = 'tcp://' + this.options.martinet_url + ':' + defaults.worker_port;
  var statusConnection = 'tcp://' + this.options.martinet_url + ':' + defaults.status_port;
...
Worker.prototype.setComplete = function(task, data) {
  this.workerSocket.send(JSON.stringify({task: task, set: 'complete', result: data}));
};

Worker.prototype.setError = function(task, error) {
  this.statusSocket.send(JSON.stringify({task: task, set: 'error', error: error}));
};

Worker.prototype.setProgress = function(task, progress) {
  this.statusSocket.send(JSON.stringify({task: task, set: 'progress', progress: progress}));
};
